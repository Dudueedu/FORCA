...
void jogo(int vidas)
{
    ...
    while (lifes != 0)
    {
        personagem(lifes);
        gotoxy(55,2);
        printf("REMAINING LIVES: %d", lifes);
        for (int i = 0; i < 15; i++)
        {
            tentativa[i] = NULL; // Limpando o lixo da array para toda tentativa
        }
        gotoxy(35, 10);
        printf("Letters already chosen: %s", palavra); 
        gotoxy(35, 13);
        printf("LETTER OR WORD: ");
